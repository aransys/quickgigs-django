...
class TestGigModel:
    ...
    def test_slug_is_autogenerated(self, gig):
        assert gig.slug.startswith("build-a-landing-page")

    def test_slug_uniqueness(self, employer):
        first = Gig.objects.create(
            title="Dup", description="x" * 10, employer=employer,
            budget=Decimal("10"), location="Remote", category=Gig.Category.OTHER,
        )
        second = Gig.objects.create(
            title="Dup", description="x" * 10, employer=employer,
            budget=Decimal("10"), location="Remote", category=Gig.Category.OTHER,
        )
